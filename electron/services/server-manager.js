...
class ServerManager {
  ...
  _runMigrations() {
    ...
    // Si no hay ningún registro todavía (fresh install o base existente sin historial),
    // el schema_limpio.sql ya cubre todo — sellar todas las migraciones actuales.
    if (ran.size === 0) {
      const values = files.map(f => `(${JSON.stringify(f)})`).join(',');
      if (values) {
        mysql(['-e', `INSERT IGNORE INTO _schema_migrations (nombre) VALUES ${values};`],
          { stdio: 'ignore' });
      }
      const reason = 'schema ya aplicado';
      console.log(`[migrations] ${reason} — ${files.length} migración(es) selladas.`);
      return;
    }
  ...
  // Write api/config/db.local.php so the installer wizard skips the DB step.
  // Env vars in startPhpServer take priority at runtime; this file lets PHP's
  // estaConfigurado() return true so the wizard skips the DB-connection step.
  _writeDbLocal() {
    const dbLocalPath = path.join(this._appDir, 'api', 'config', 'db.local.php');
    const content = [
      '<?php',
